...
- **File output**: Weekly overwrite to simulate upsert (no duplicates)

## Output Proof

- **Confluence**: `outputs/confluence/{YYYY-Month}/week-{WW}.md`
- **Confluence index (monthly rollover log)**: `outputs/confluence/{YYYY-Month}/index.md`
- **Slide**: `outputs/slides/{YYYY}-W{WW}.txt`
- **Verification summary**: `outputs/verification_summary_{YYYY}-W{WW}.md`

## If We Had More Time…

- **Confluence API upsert**: Replace file writes with page create/update logic and section-level weekly updates.
- **Google Slides API upsert**: Create or update the week’s slide instead of producing text output.
- **Live Jira integration**: Replace mock data with JQL-driven Jira API ingestion plus pagination and auth.
- **Production-grade error handling**: Retries, alerting, and run-level logging for failed nodes.
- **Automated monthly rollover**: Server-side Confluence page rollover with explicit archive/index updates.
